---
title: "Analysis of the tweets related to the brand Airbnb"
author: "Enas Alahmadi"
...
Before I start the analysis, I must install the packages needed. 


Some of the libraries are not supported by CRAN so using install.packages() won't work. We need to use the "devtools" package to install them from a url as shown below.
...
### Analysis of the word frequency data frame

For this part I analyze the mean, median, mode, minimum, maximum, and the quantiles of the words associated with Airbnb frequencies. From the results I can say that the data is skewed to the right because the mode is less than the mean, which is less than the median. This is also clear when observing the quantiles. The range of the words' frequencies is large, which is expected.


```{r}
The.mean = mean(fr.clean$freq)
...
The.median = median(fr.clean$freq)
The.median
The.quantile = quantile(fr.clean$freq, c(0.05,0.1,0.25,0.5,0.75,0.9,0.95))
The.quantile
minimum.value = min(fr.clean$freq)
minimum.value
maximum.value = max(fr.clean$freq)
maximum.value
range.freq = maximum.value - minimum.value
range.freq
standard.deviation = sd(fr.clean$freq)
standard.deviation
...
In this part of the report I use the function sentimentalTabel() to analyze the sentiment of the tweets collected. Since I acquired a large number of tweets, the sentimental analysis of the dataset I have should reflect the population's. 

It is clear that most of the tweets about Airbnb are positive, which indicates satisfaction of the services they provide.
